...
Implementation update, 2026-09-24: milestone 1's Type Cover detection is implemented behind an off-by-default setting (issue #2). The SP4 cover is a USB HID device (045e:07e8) with no tablet-mode switch; the kernel log shows one clean disconnect per detach and one enumeration per reattach. One logged resume dropped it for about 2.5 seconds. Real detach, reattach, and suspend tests with the setting on remain to be done by hand.

Implementation update, 2026-09-24 (later): folding the Type Cover behind the screen now enters tablet mode, through an optional, opt-in fold helper (`install.sh --with-fold-helper`). The cover reports its fold position in byte 1 of vendor input report 35 on its root-only hidraw node, which also carries keystrokes; a small C service reads that one byte with a descriptor handed to it by systemd, runs as a dynamic user under the unit's sandbox and its own seccomp filter (ioctl only as GET_REPORT), and publishes a single word under `/run`. Its tests, a sandbox test under the unit's syscall filter, and an offline simulation of the Lua transitions pass. Installing it and the fold, unfold, and suspend checks in TESTING.md remain to be done on the device.
