...
-- The SP4 cover settles on `between` (0x33) when folded back, measured;
-- 0x43 is only seen for a few seconds. So `between` held for 2 s is folded,
-- and a shorter one (lifting the cover on the way round) changes nothing.
fold = "between"; run(1.5)
expect("between for 1.5 s from typing: no flip", "laptop", "attached")
fold = "typing"; run(1)
expect("back to typing: still laptop", "laptop", "attached")
fold = "between"; run(3)
expect("between held from typing -> tablet", "tablet", "folded")
fold = "typing"; run(1)
expect("unfold from between -> laptop", "laptop", "attached")
fold = "folded"; run(0.5)
...
fold = "between"; run(1)
expect("... and holds through a short between", "tablet", "attached")
fold = "folded"; run(1)
